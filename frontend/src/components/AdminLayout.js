...
const AdminLayout = ({ children }) => {
  ...
  const navItems = [
    ...
    { name: 'Restock', href: '/admin/restock', icon: BuildingStorefrontIcon },
    { name: 'Sales Report', href: '/admin/report', icon: ChartPieIcon },
    { name: 'Manage Prices', href: '/admin/prices', icon: SunIcon },
  ...
  const SidebarContent = ({ collapsed, mobile }) => (
    <>
      <button
        onClick={() => !mobile && setIsCollapsed(!collapsed)}
        className={`absolute -right-3 top-9 rounded-full p-1 shadow-md border transition-colors z-10 ${
          mobile ? 'hidden' : ''
        } ${
          isDark
      ...
      <div className={`flex items-center mb-8 h-8 flex-shrink-0 px-2 ${mobile ? 'mt-4' : ''}`}>
        <div className={`flex items-center gap-2 transition-opacity duration-300 ${collapsed ? 'opacity-0' : 'opacity-100'}`}>
          ...
          <h1 className={`text-xl font-black tracking-tighter whitespace-nowrap ${isDark ? 'text-white' : 'text-gray-900'}`}>
            ADMIN PANEL
          </h1>
        </div>
        {mobile && (
