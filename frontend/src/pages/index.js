    ...
    <div className={`relative min-h-screen overflow-x-hidden ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
      ...
      <section className="relative pt-20 pb-20 md:pt-32 md:pb-32 px-4">
        ...
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
          ...
          <div className="absolute top-[20%] -right-[5%] w-[30%] h-[30%] bg-emerald-500/10 rounded-full blur-[100px]" />
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
          <motion.div 
            ...
            className="space-y-8"
          >
            <div className="inline-flex items-center px-4 py-2 rounded-full bg-indigo-500/10 text-indigo-500 text-[10px] font-black uppercase tracking-[0.3em]">
            ...
            <h1 className={`text-5xl md:text-7xl font-black leading-[1.1] tracking-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>
              Refreshment, <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-emerald-500">Synchronized.</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/dashboard">
                <motion.div
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-2xl shadow-indigo-500/40 flex items-center justify-center gap-2 cursor-pointer"
                >
                  Start Ordering Now
      ...
      <section className={`py-24 px-4 ${isDark ? 'bg-gray-800/30' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            />
            <FeatureCard 
              icon={<ShieldCheckIcon className="h-8 w-8 text-emerald-500" />}
              title="Secure Checkout"
              desc="Integrated directly with M-Pesa STK push for reliable transactions."
              isDark={isDark}
            />
            <FeatureCard 
              icon={<MapPinIcon className="h-8 w-8 text-amber-500" />}
              title="Local Access"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="order-1 lg:order-2 space-y-8">
            ...
            <Link href="/dashboard">
              <motion.div 
                whileHover={{ x: 10 }}
                className="inline-flex items-center gap-2 text-indigo-500 font-black uppercase text-xs tracking-[0.2em] cursor-pointer"
              >
                Launch Dashboard
                <ArrowRightIcon className="h-4 w-4 stroke-[3px]" />
              </motion.div>
      ...
      <section className="px-4 pb-24 pt-12">
        <div className={`max-w-7xl mx-auto rounded-[3rem] p-12 md:p-24 text-center relative overflow-hidden bg-indigo-600 shadow-2xl shadow-indigo-500/40`}>
          <div className="relative z-10 space-y-8">
            ...
            <Link href="/register">
              <motion.div
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="inline-block px-12 py-5 bg-white text-indigo-600 rounded-2xl font-black uppercase text-xs tracking-[0.3em] shadow-xl cursor-pointer"
              >
          ...
          <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/4 w-64 h-64 bg-emerald-400/20 rounded-full" />
        </div>
      </section>
      ...
      <footer className={`py-12 px-4 border-t ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex gap-6">
            <Link href="/about" className={`text-[10px] font-black uppercase tracking-widest hover:text-indigo-500 transition-colors ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>About</Link>
...
function Step({ number, title, desc, isDark }) {
  return (
    <div className="flex gap-6">
      <span className="text-4xl font-black text-indigo-500/20 tracking-tighter leading-none">{number}</span>
      <div>
