...
const ProfilePage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/auth/update`, updateData, {
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="relative overflow-hidden rounded-[2.5rem] p-8 md:p-12 border-2 border-indigo-500/10 bg-gradient-to-br from-indigo-600 to-indigo-700 shadow-2xl shadow-indigo-500/20 text-white">
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                        <div className="h-32 w-32 rounded-[2rem] bg-white/10 backdrop-blur-xl border-4 border-white/20 flex items-center justify-center shadow-inner">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className={`p-8 rounded-[2rem] border-2 transition-all ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100 shadow-sm'}`}>
                            <h3 className={`text-lg font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>Security</h3>
                            <p className={`text-sm font-medium leading-relaxed mb-6 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                Keep your account safe by ensuring your password is complex and updated regularly.
                            </p>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className={`block text-[10px] font-black uppercase tracking-[0.2em] ml-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Full Name</label>
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2 text-center md:text-left">
                                    <h4 className={`text-sm font-black uppercase tracking-widest ${isDark ? 'text-white' : 'text-gray-900'}`}>Update Password</h4>
                                    <p className={`text-[10px] font-bold uppercase tracking-widest ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Leave blank to keep existing password</p>
                                </div>
